...
class HavanoInvoicesController(HavanoApiControllerMixin, http.Controller):
    ...
    def _serialize_invoice(self, move):
        return {
            ...
            "pricelist_id": move.partner_id.property_product_pricelist.id if move.partner_id.property_product_pricelist else None,
    ...
    def _create_invoice(self, env):
        ...
        # Get customer's pricelist for price calculation
        ...
        _logger.info("Invoice created: id=%s, partner=%s, pricelist=%s, total=%s",
                    move.id, partner.name, pricelist.name if pricelist else "Default", move.amount_total)
